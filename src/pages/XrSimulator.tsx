    ...
    <article>
        ...
        <div className="grid gap-6 sm:grid-cols-3">
          {stats.map((stat) => (
            <div key={stat.label} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-4 font-display text-2xl font-bold text-slate-900">{stat.value}</p>
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-6">
        <div className="grid gap-3 sm:grid-cols-[auto_1fr] sm:gap-6">
          <div className="hidden sm:grid h-11 w-11 place-items-center rounded-xl bg-slate-900 text-white">
            <Monitor size={22} />
          </div>
          <div>
            <h2 className="font-display text-2xl font-bold text-slate-900">The problem</h2>
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h2 className="font-display text-2xl font-bold text-slate-900">What I built</h2>
        <div className="mt-8 space-y-8">
          {highlights.map((h, i) => (
            <div key={h.title} className="flex gap-5">
              <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-indigo-600 font-mono text-sm font-bold text-white">
                {i + 1}
              </div>
              <div>
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-8">
          <h2 className="font-display text-2xl font-bold text-slate-900">Impact</h2>
          <p className="mt-3 leading-relaxed text-slate-600">
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-slate-200 pt-8">
          <Link to="/experience" className="text-sm font-semibold text-slate-600 hover:text-slate-900">
            ← Back to experience
          </Link>
          <Link
            to="/contact"
